    ...
    <section className="relative overflow-hidden py-24 md:py-32 font-sans bg-gradient-to-b from-white via-gray-50/50 to-white">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ overflow: "visible" }}>
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ scale: { duration: 0.6 }, opacity: { duration: 0.6 } }}
        />
        <motion.div
          ...
          whileInView={{ scale: 1, opacity: 1 }}
          viewport={{ once: true }}
          transition={{ scale: { duration: 0.6, delay: 0.2 }, opacity: { duration: 0.6, delay: 0.2 } }}
        />
      </div>
      ...
      <div className="max-w-[1920px] mx-auto relative z-10 px-6">
        ...
        <div className="flex flex-col items-center text-center mb-16">
           <SectionTitle
            ...
            highlightEnd={true} 
          />
          <p className="text-gray-600 max-w-2xl mt-4 text-sm md:text-base">
            Discover how much revenue you could be generating with your current subscriber base using our quick calculator.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div className="p-8 md:p-10 space-y-10 border-b lg:border-b-0 lg:border-r border-gray-100">
                    ...
                    <div className="bg-gray-100 p-1.5 rounded-xl flex relative w-fit mx-auto lg:mx-0">
                        <button
                            ...
                            className={cn(
                                ...
                                isPaid ? "text-white shadow-md" : "text-gray-500 hover:text-gray-900"
                            )}
                        >
                            Paid
                            {isPaid && <motion.div layoutId="bubble" className="absolute inset-0 bg-theme-brand rounded-lg -z-10" />}
                        </button>
                        <button
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-end">
                            ...
                            <span className="text-2xl font-black text-gray-900">{subscribers[0].toLocaleString()}</span>
                        </div>
                        <Slider
                            value={subscribers}
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-end">
                            ...
                            <span className="text-2xl font-black text-gray-900">${isPaid ? price[0] : 0}</span>
                        </div>
                        <Slider
                            value={price}
                ...
                <div className="p-8 md:p-10 bg-gray-50/50 flex flex-col justify-between space-y-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {[1, 2, 3].map((tier) => (
                                ...
                                >
                                    {tier === 1 ? "1st World" : tier === 2 ? "2nd World" : "3rd World"}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="flex justify-center gap-2 h-10 items-center">
                             {locationTier === 1 && (
                                <div className="flex gap-2 animate-in fade-in slide-in-from-bottom-2">
                                  {["us", "gb", "ca", "au", "de", "ch", "se", "fi", "nz"].map(code => <Flag key={code} countryCode={code} />)}
                                </div>
                             )}
                             {locationTier === 2 && (
                                <div className="flex gap-2 animate-in fade-in slide-in-from-bottom-2">
                                  {["ru", "ua", "cz", "pl", "al", "cn", "kp", "vn"].map(code => <Flag key={code} countryCode={code} />)}
                                </div>
                             )}
                             {locationTier === 3 && (
                                <div className="flex gap-2 animate-in fade-in slide-in-from-bottom-2">
                                  {["br", "bo", "cl", "co", "mx", "ph", "sa", "th"].map(code => <Flag key={code} countryCode={code} />)}
                                </div>
                             )}
                        </div>
                    </div>
                        ...
                        <label className="flex items-center gap-2 text-gray-700 font-bold text-sm uppercase tracking-wide">
                         ...
                         <div className="grid grid-cols-3 gap-2">
                            {["A", "B", "C"].map((grade) => (
                                <button
                                    key={grade}
                                    onClick={() => setTrafficQuality(grade as "A"|"B"|"C")}
                                    className={cn(
                                ...
                                >
                                    Grade {grade}
                                    <Info size={12} className={cn(trafficQuality === grade ? "text-theme-brand" : "text-gray-400")} />
                                </button>
                            ))}
                        </div>
                        {/* Traffic Icons */}
                        <div className="flex gap-4 h-10 items-center justify-center">
                             {trafficQuality === "A" && (
                                 <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-2">
                                     <div className="bg-red-50 p-1.5 rounded-full border border-red-100"><Icons.YouTube className="w-6 h-6 text-red-600" /></div>
                                     <div className="bg-purple-50 p-1.5 rounded-full border border-purple-100"><Icons.Twitch className="w-6 h-6 text-purple-600" /></div>
                                 </div>
                             )}
                             {trafficQuality === "B" && (
                                 <div className="flex gap-3 animate-in fade-in slide-in-from-bottom-2">
                                     <div className="bg-pink-50 p-1.5 rounded-full border border-pink-100"><Icons.Instagram className="w-5 h-5 text-pink-600" /></div>
                                     <div className="bg-gray-50 p-1.5 rounded-full border border-gray-200"><Icons.Threads className="w-5 h-5 text-black" /></div>
                                     <div className="bg-orange-50 p-1.5 rounded-full border border-orange-100"><Icons.Reddit className="w-5 h-5 text-orange-600" /></div>
                                     <div className="bg-gray-50 p-1.5 rounded-full border border-gray-200"><Icons.X className="w-5 h-5 text-black" /></div>
                                 </div>
                             )}
                             {trafficQuality === "C" && (
                                 <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="mt-auto bg-white rounded-2xl p-6 border border-gray-200 shadow-sm text-center space-y-2">
                        <p className="text-gray-500 font-semibold text-sm uppercase tracking-wider">Estimated Monthly Earnings</p>
                        <motion.div
                        ...
                        <p className="text-xs text-theme-brand font-medium">*Estimated PPV / Tips Revenue</p>
                    </div>
